                    ...
                    <div class="flex-1 pl-2 pr-2 py-2 bg-white rounded-r-lg border-2 border-white shadow-md">
                        ...
                        <div class="grid grid-cols-2 gap-y-1">
                            ...
                            <div class="flex flex-col text-left">
                                ...
                                <div class="flex items-center space-x-1">
                                    <i class="fas fa-arrow-up text-green-500 text-[11px]"></i>
                                    <p class="text-green-500 text-[11px]">{{ roundToTwo(dept.actualProductionGold || 0) }}</p>
                                </div>
                                <div class="flex items-center space-x-1">
                                    <i class="fas fa-arrow-down text-red-500 text-[11px]"></i>
                                    <p class="text-red-500 text-[11px]">{{ roundToTwo(dept.actualLossGold || 0) }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col">
                                ...
                                <div class="flex items-center space-x-1">
                                    <i class="fas fa-arrow-up text-green-500 text-[11px]"></i>
                                    <p class="text-green-500 text-[11px]">{{ roundToTwo(dept.actualProductionDiamond || 0) }}</p>
                                </div>
                                <div class="flex items-center space-x-1">
                                    <i class="fas fa-arrow-down text-red-500 text-[11px]"></i>
                                    <p class="text-red-500 text-[11px]">{{ roundToTwo(dept.actualLossDiamond || 0) }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <tbody class="text-gray-700">
                            ...
                            <template v-if="!showEmployeesTable" v-for="(dept, deptIndex) in selectedDepartmentData" :key="'dept-' + deptIndex">
                                ...
                                <tr v-else class="border-b group hover:bg-gray-50 transition-all duration-200 text-[11px]">
                                    ...
                                    <!-- Starting Qty Gold -->
                                    <td class="px-3 py-2 group-hover:shadow-md">{{ roundToTwo(dept.starting_quantity_gold || 0) }}</td>
                                    
                                    <!-- TM Production Gold -->
                                    <td class="px-3 py-2 group-hover:shadow-md">{{ roundToTwo(dept.production || 0) }}</td>
                                    
                                    <!-- Actual Production Gold -->
                                    <td class="px-3 py-2 group-hover:shadow-md">{{ roundToTwo(dept.actualProductionGold || 0) }}</td>
                                    
                                    <!-- Gross Loss Gold -->
                                    <td class="px-3 py-2 text-red-500 group-hover:shadow-md">{{ roundToTwo(dept.loss || 0) }}</td>
                                    
                                    <!-- Gold Loss % -->
                                    <td class="px-3 py-2 text-red-500 group-hover:shadow-md">{{ roundToTwo(calculateGoldLossPercentage(dept.actualProductionGold, dept.loss)) }}%</td>
                                    
                                    <!-- Starting Qty Diamond -->
                                    <td class="px-3 py-2 group-hover:shadow-md">{{ roundToTwo(dept.starting_quantity_diamond || 0) }}</td>
                                    
                                    <!-- TM Production Diamond-->
                                    <td class="px-3 py-2 group-hover:shadow-md">{{ roundToTwo(dept.production_diamond || 0) }}</td>
                                    
                                    <!-- Actual Production Diamond -->
                                    <td class="px-3 py-2 group-hover:shadow-md">{{ roundToTwo(dept.actualProductionDiamond || 0) }}</td>
                                    
                                    <!-- Gross Loss Diamond -->
                                    <td class="px-3 py-2 text-red-500 group-hover:shadow-md">{{ roundToTwo(dept.loss_diamond || 0) }}</td>
                                    
                                    <!-- Diamond Loss % -->
                                    <td class="px-3 py-2 text-red-500 group-hover:shadow-md">{{ roundToTwo(calculateDiamondLossPercentage(dept.actualProductionDiamond, dept.loss_diamond)) }}%</td>
                        ...
                        <tbody class="text-gray-700">
                            <template v-if="showEmployeesTable" v-for="(emp, empIndex) in selectedEmployees" :key="'emp-' + empIndex">
                                <tr class="border-b group hover:bg-gray-50 transition-all duration-200 text-[11px]">
                                    ...
                                    <!-- No. of Bags -->
                                    <td class="px-3 py-2 font-semibold text-center bg-blue-50">{{ emp.bagCount || 0 }}</td>
    ...
    setup(props) {
        ...
        const totalDeptActualProductionGold = computed(() => {
            ...
            selectedDepartmentData.value.forEach(dept => {
                sum += parseFloat(dept.actualProductionGold || 0);
            });
        ...
        const totalDeptActualProductionDiamond = computed(() => {
            let sum = 0;
            selectedDepartmentData.value.forEach(dept => {
                sum += parseFloat(dept.actualProductionDiamond || 0);
            });
            return roundToTwo(sum);
        });
        ...
        // Helper function to calculate gold loss percentage
        const calculateGoldLossPercentage = (productionGold, lossGold) => {
            const total = (productionGold || 0) + (lossGold || 0);
            if (total === 0) return 0;
            return ((lossGold || 0) / total) * 100;
        };

        // Helper function to calculate diamond loss percentage
        const calculateDiamondLossPercentage = (productionDiamond, lossDiamond) => {
            const total = (productionDiamond || 0) + (lossDiamond || 0);
            if (total === 0) return 0;
            return ((lossDiamond || 0) / total) * 100;
        };
        ...
        const fetchEfficiencyAnalysisData = async (locationId = null) => {
            try {
                ...
                if (listEfficiencyData.value && Object.keys(listEfficiencyData.value).length > 0) {
                    ...
                    console.log('=== FETCHED EFFICIENCY DATA ===');
                    console.log('Full Data:', JSON.parse(JSON.stringify(listEfficiencyData.value)));
                    ...
                    if (!locationId) {
                        ...
                        locations.value = Object.entries(listEfficiencyData.value).map(([locId, locData]) => ({
                            ...
                            loss_diamond_pieces: locData.loss_diamond_pieces || 0,
                            departments: Object.entries(locData.departments || {}).map(([deptId, dept]) => {
                                // ✅ Compute department gold recovery weight **only from its employees**
                                const totalEmployeeRecoveryWeight = Object.entries(dept.employees || {}).reduce((sum, [empId]) => {
                                    const key = `${deptId}_${empId}`;
                                    return sum + (employeeQuantities[key] || 0);
                                }, 0);

                                const grossLossGoldDept = parseFloat(dept.loss || 0);
                                const netLossGoldDept = grossLossGoldDept - totalEmployeeRecoveryWeight;
                                ...
                                return {
                                    ...
                                    starting_quantity_gold: dept.starting_quantity_gold || 0,
                                    issued_quantity_diamond: dept.issued_quantity_diamond || 0,
                                    loss_quantity_diamond: dept.loss_quantity_diamond || 0,
                                    starting_quantity_diamond: dept.starting_quantity_diamond || 0,
                    ...
                    retryCount = 0; // Reset retry count when we get valid data
                } else {
                    if (retryCount < maxRetries) {
                        retryCount++; // Increment retry count
                        console.warn("No data found. Retrying with default date range...");

                        showNoDataPopup.value = true;
                        setTimeout(() => {
                            showNoDataPopup.value = false;
                            selectedDateRange.value = getDefaultDateRangeLast();
                            if (retryCount >= maxRetries) {
                                showNoDataMessage.value = true; // Show no data message after retries
                            }
                        }, 3000);
                    }
                }
        ...
        const toggleLocationView = async (locationId) => {
            if (expandedLocation.value == locationId) {
                ...
                console.log("Departments with Employee Bag Counts:", selectedDepartments.value);
                updateCharts();
            }
        };
        ...
        const toggleDepartmentView = (deptName) => {
            if (expandedDepartment.value === deptName) {
                ...
                console.log(`Department: ${deptName} (ID: ${selectedDept?.id}) - Employees with Bag Counts:`, selectedEmployees.value);
            }
            updateCharts();
        };
